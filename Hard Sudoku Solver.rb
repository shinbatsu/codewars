...
def invalid(g) g.size-9|| g.any? {|r| r.any? {|v| !D.include?(v) }}|| g.transpose.any?{ |col| col.uniq.size != 9 }
end
...
def f(a, c, &b)
  ...
  pos, vals =c.min_by { |x, v| v.size }
  vals.each do|val|
    next_c = c.reject{ |p, x| p == pos }.map { |p, vs| [p, related(pos, p) ? vs - Set[val] : vs] }
    f(a + [[pos, val]], next_c, &b)
  end
end
def solve(g)
  sols =f([], q(g)).lazy; s = sols.next
  raise 'NO UNIQUE!' if invalid(g)|| sols.next rescue false
  gridify(s)
end